...
def draw_moves(moves):
    for move in moves:
        pygame.draw.circle(
            ...
            GREY,
            (
                (BOARD_OFFSET + move[0] * SQSIZE) + SQSIZE // 2,
                (BOARD_OFFSET + move[1] * SQSIZE) + SQSIZE // 2,
            ),
            SQSIZE // 5,
        )
...
def draw_column_letters():
    for c in range(COLS):
        ...
        window.blit(
            text,
            ((BOARD_OFFSET + SQSIZE - 20) + c * SQSIZE, HEIGHT - BOARD_OFFSET - 25),
        )
...
def draw_pieces(board):
    for r in range(ROWS):
        for c in range(COLS):
            ...
            if piece:
                window.blit(
                    piece.img, (BOARD_OFFSET + c * SQSIZE, BOARD_OFFSET + r * SQSIZE)
                )
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            pygame.draw.rect(
                window,
                color,
                (BOARD_OFFSET + c * SQSIZE, BOARD_OFFSET + r * SQSIZE, SQSIZE, SQSIZE),
            )
